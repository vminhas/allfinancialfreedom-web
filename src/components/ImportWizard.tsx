...
// Shared PropHog CSV import wizard. Rendered standalone on /vault/import
// and embedded on the Outreach dashboard. `embedded` drops the page
// chrome so it sits inside a card; `onComplete` fires after a finished
// import so the host can refresh its contact list.
export default function ImportWizard({
  ...
  async function handleImport() {
    ...
    // The server processes ~45s per request then hands back; poll the
    // job row (counters are flushed per batch) so the bar moves the
    ...
    try {
      // Drain the import in resumable, time-boxed chunks. A single
      // request can't process thousands of contacts before the
      // serverless timeout, so we loop until the server reports done
      // or blocked by the daily cap.
      let guard = 0
      while (guard++ < 1000) {
        ...
        if (res.status === 409) {
          // Another invocation is genuinely in-flight; wait and retry.
          await new Promise(r => setTimeout(r, 3000))
          continue
        }
        if (!res.ok) {
          setImportProgress(p => ({ imported: p?.imported ?? 0, total: jobStats?.toImport ?? 0, status: 'ERROR' }))
          break
        }
        ...
        if (data.dailyCap) {
          setImportProgress(p => ({ imported: p?.imported ?? (data.imported as number) ?? 0, total: jobStats?.toImport ?? 0, status: 'PAUSED' }))
          break
        }
        if (data.hasMore) {
        ...
        // Complete.
        setImportProgress(p => ({ imported: p?.imported ?? (data.imported as number) ?? 0, total: jobStats?.toImport ?? 0, status: 'COMPLETE' }))
        setStep('done')
        onComplete?.()
        break
      }
  ...
  return (
    <div style={embedded ? undefined : { maxWidth: 760 }}>
      {!embedded && (
      ...
      {/* Step indicators */}
      <div style={{ display: 'flex', gap: 8, marginBottom: embedded ? 24 : 36 }}>
      ...
      {step === 'upload' && (
        <div
          ...
          style={{
            ...
            borderRadius: 8, padding: embedded ? '36px 40px' : '60px 40px', textAlign: 'center', cursor: 'pointer',
          ...
          <div style={{ padding: '28px' }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ color: '#6B8299', fontSize: 11, margin: '6px 0 0' }}>
                Optional — also editable on the Outreach page later.
              </p>
            </div>
      ...
      {step === 'import' && importProgress && (
        <div style={{ background: '#142D48', borderRadius: 6, border: '1px solid rgba(201,169,110,0.1)', padding: '40px 48px', textAlign: 'center' }}>
          <p style={{ color: importProgress.status === 'ERROR' ? '#f87171' : '#C9A96E', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 600, margin: '0 0 16px' }}>
            {importProgress.status === 'RUNNING' ? 'Importing...'
              : importProgress.status === 'PAUSED' ? 'Daily Limit Reached'
              : importProgress.status === 'ERROR' ? 'Import Error'
              : 'Complete'}
          </p>
          ...
          {importProgress.status === 'PAUSED' && (
            <p style={{ color: '#f59e0b', fontSize: 13, margin: '12px 0 0' }}>
              Daily limit of 2,400 reached. The rest stays queued &middot; resume tomorrow or by clicking Import again.
            </p>
          )}
          {importProgress.status === 'ERROR' && (
            <div style={{ marginTop: 14 }}>
              <p style={{ color: '#f87171', fontSize: 13, margin: '0 0 12px' }}>
                The import hit an error. Progress is saved &middot; click resume to continue from where it stopped.
              </p>
              <button onClick={handleImport} disabled={importing} style={{
      ...
      {/* Step: Done */}
      {step === 'done' && (
        <div style={{ background: '#142D48', borderRadius: 6, border: '1px solid rgba(74,222,128,0.2)', padding: embedded ? '32px' : '48px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#6B8299', fontSize: 13, margin: '0 0 28px' }}>
            Contacts are in GHL and your local database{embedded ? ' — they appear in the list below.' : '.'}
          </p>
