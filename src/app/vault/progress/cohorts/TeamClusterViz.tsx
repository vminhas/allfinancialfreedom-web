...
export default function TeamClusterViz({ rows, onSelect }: { rows: AgentProgress[]; onSelect: (k: BlockerKey) => void }) {
  ...
  useEffect(() => {
    ...
    const circ = svg.append('g').selectAll<SVGCircleElement, Node>('circle').data(nodes).join('circle')
      .attr('r', 6.6).attr('fill', d => BLOCKER_META[d.r.blocker].color).attr('stroke', '#fff').attr('stroke-width', 1.3)
      .attr('cx', d => d.x ?? 0).attr('cy', d => d.y ?? 0).style('cursor', 'pointer')
      .on('click', (_e, d) => onSelect(d.r.blocker))
    circ.append('title').text(d => `${d.r.agent.firstName} ${d.r.agent.lastName} · ${BLOCKER_META[d.r.blocker].label}`)
  }, [rows, onSelect])
